...
### 1. Luồng Trạng Thái Kiểm Duyệt Trang Trại (Farms Lifecycle)
...
*   **Lớp Cache 1 (Local Memory Cache - Caffeine):** Lưu trữ trên RAM cục bộ của JVM. Áp dụng cho dữ liệu ít thay đổi nhưng tần suất đọc cực cao (Danh mục sản phẩm, danh sách Tỉnh/Thành giao hàng).
*   **Lớp Cache 2 (Distributed Cache - Redis):** Lưu trữ phân tán bên ngoài ứng dụng. Áp dụng cho dữ liệu có tần suất thay đổi trung bình (Thông tin chi tiết sản phẩm, số lượng tồn kho khả dụng). Đảm bảo tính nhất quán dữ liệu (Data Consistency) khi chạy ứng dụng trên nhiều container song song.
